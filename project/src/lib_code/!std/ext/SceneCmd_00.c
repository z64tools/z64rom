...
void func_80098508(PlayState* playState, SceneCmd* cmd) {
	ActorEntry* linkEntry = playState->linkActorEntry = (ActorEntry*)SEGMENTED_TO_VIRTUAL(cmd->spawnList.segment) + playState->setupEntranceList[playState->curSpawn].spawn;
	s16 linkObjectId;
