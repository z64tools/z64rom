...
s16 sFileInfoBoxPartWidths[] = { 36, 36, 36, 36, 24 };
...
void FileSelect_SetView(FileChooseContext* this, f32 eyeX, f32 eyeY, f32 eyeZ) {
	Vec3f eye;
	Vec3f lookAt;
	Vec3f up;
...
void FileSelect_UpdateMainMenu(GameState* thisx) {
	u8 emptyName[] = { 0x3E, 0x3E, 0x3E, 0x3E, 0x3E, 0x3E, 0x3E, 0x3E };
	FileChooseContext* this = (FileChooseContext*)thisx;
	SramContext* sramCtx = &this->sramCtx;
	...
	if (CHECK_BTN_ALL(input->press.button, BTN_START) || CHECK_BTN_ALL(input->press.button, BTN_A)) {
		if (this->buttonIndex <= FS_BTN_MAIN_FILE_3) {
			osSyncPrintf(
				"REGCK_ALL[%x]=%x,%x,%x,%x,%x,%x\n",
				this->buttonIndex,
				GET_NEWF(sramCtx, this->buttonIndex, 0),
				GET_NEWF(sramCtx, this->buttonIndex, 1),
				GET_NEWF(sramCtx, this->buttonIndex, 2),
				GET_NEWF(sramCtx, this->buttonIndex, 3),
				GET_NEWF(sramCtx, this->buttonIndex, 4),
	...
	if (CHECK_BTN_ALL(debugInput->press.button, BTN_DLEFT)) {
		...
		*((u8*)0x80000002) = LANGUAGE_ENG;
		...
		osSyncPrintf(
			...
			sramCtx->readBuff[SRAM_HEADER_ZTARGET],
			sramCtx->readBuff[SRAM_HEADER_LANGUAGE],
...
u16 D_8081284C[] = { 0x007C, 0x0124, 0x01CC };
...
s16 sQuestItemRed[] = { 255, 255, 255, 0, 255, 0, 255, 200, 200 };
s16 sQuestItemGreen[] = { 255, 255, 255, 255, 60, 100, 130, 50, 200 };
s16 sQuestItemBlue[] = { 255, 255, 255, 0, 0, 255, 0, 255, 0 };
s16 sQuestItemFlags[] = { 0x0012, 0x0013, 0x0014, 0x0000, 0x0001, 0x0002, 0x0003, 0x0004, 0x0005 };
s16 sNamePrimColors[2][3] = { { 255, 255, 255 }, { 100, 100, 100 } };
void* sHeartTextures[] = { &gHeartFullTex, &gDefenseHeartFullTex };
s16 sHeartPrimColors[2][3] = { { 255, 70, 50 }, { 200, 0, 0 } };
s16 sHeartEnvColors[2][3] = { { 50, 40, 60 }, { 255, 255, 255 } };
...
void FileSelect_DrawFileInfo(GameState* thisx, s16 fileIndex, s16 isActive) {
	...
	if (this->nameAlpha[fileIndex] != 0) {
		...
		gDPSetPrimColor(
			...
			0x00,
			0x00,
			sNamePrimColors[isActive][0],
			sNamePrimColors[isActive][1],
			sNamePrimColors[isActive][2],
			this->nameAlpha[fileIndex]
		);
		
		for (i = 0, vtxOffset = 0; vtxOffset < 0x20; i++, vtxOffset += 4) {
			FileSelect_DrawCharacter(
				this->state.gfxCtx,
				sp54->fontBuf + this->fileNames[fileIndex][i] * FONT_CHAR_TEX_SIZE,
				vtxOffset
			);
		}
	}
	...
	if ((fileIndex == this->selectedFileIndex) || (fileIndex == this->copyDestFileIndex)) {
		...
		for (i = 0, vtxOffset = 0; i < 3; i++, vtxOffset += 4) {
			FileSelect_DrawCharacter(
				this->state.gfxCtx,
				sp54->fontBuf + deathCountSplit[i] * FONT_CHAR_TEX_SIZE,
				vtxOffset
			);
		}
		...
		gDPSetPrimColor(
			...
			0x00,
			0x00,
			sHeartPrimColors[heartType][0],
			sHeartPrimColors[heartType][1],
			sHeartPrimColors[heartType][2],
			this->fileInfoAlpha[fileIndex]
		);
		gDPSetEnvColor(
			POLY_OPA_DISP++,
			sHeartEnvColors[heartType][0],
			sHeartEnvColors[heartType][1],
			sHeartEnvColors[heartType][2],
			255
		);
...
void* sActionButtonTextures[3][4] = {
...
void FileSelect_DrawWindowContents(GameState* thisx) {
	...
	gDPLoadTextureBlock(
		POLY_OPA_DISP++,
		sTitleLabels[gSaveContext.language][this->nextTitleLabel],
		G_IM_FMT_IA,
		G_IM_SIZ_8b,
		128,
		16,
	...
	for (fileIndex = 0; fileIndex < 3; fileIndex++, temp += 20) {
		...
		for (quadVtxIndex = 0, i = 0; i < 5; i++, quadVtxIndex += 4) {
			gDPLoadTextureBlock(
				POLY_OPA_DISP++,
				sFileInfoBoxTextures[i],
				G_IM_FMT_IA,
				G_IM_SIZ_16b,
				sFileInfoBoxPartWidths[i],
				56,
				0,
				G_TX_NOMIRROR | G_TX_WRAP,
				G_TX_NOMIRROR | G_TX_WRAP,
	...
	for (i = 0; i < 3; i++, temp += 20) {
		...
		gDPSetPrimColor(
			...
			0,
			0,
			sWindowContentColors[isActive][0],
			sWindowContentColors[isActive][1],
			sWindowContentColors[isActive][2],
			this->fileButtonAlpha[i]
		);
		gDPLoadTextureBlock(
			POLY_OPA_DISP++,
			sFileButtonTextures[gSaveContext.language][i],
			G_IM_FMT_IA,
			G_IM_SIZ_16b,
			64,
			16,
	...
	for (quadVtxIndex = 0, i = 0; i < 2; i++, quadVtxIndex += 4) {
		...
		gDPLoadTextureBlock(
			POLY_OPA_DISP++,
			sActionButtonTextures[gSaveContext.language][i],
			G_IM_FMT_IA,
			G_IM_SIZ_16b,
			64,
			16,
	...
	for (quadVtxIndex = 0, i = 0; i < 2; i++, quadVtxIndex += 4) {
		...
		gDPLoadTextureBlock(
			POLY_OPA_DISP++,
			sActionButtonTextures[gSaveContext.language][temp],
			G_IM_FMT_IA,
			G_IM_SIZ_16b,
			64,
			16,
	...
	gDPLoadTextureBlock(
		POLY_OPA_DISP++,
		sOptionsButtonTextures[gSaveContext.language],
		G_IM_FMT_IA,
		G_IM_SIZ_16b,
		64,
		16,
	...
	if (this->warningLabel > FS_WARNING_NONE) {
		...
		gDPLoadTextureBlock(
			POLY_OPA_DISP++,
			sWarningLabels[gSaveContext.language][this->warningLabel],
			G_IM_FMT_IA,
			G_IM_SIZ_8b,
			128,
			16,
...
void FileSelect_ConfigModeDraw(GameState* thisx) {
	...
	if ((this->configMode != CM_NAME_ENTRY) && (this->configMode != CM_START_NAME_ENTRY)) {
		...
		gSPMatrix(
			POLY_OPA_DISP++,
			Matrix_NewMtx(this->state.gfxCtx, "../z_file_choose.c", 2282),
	...
	if ((this->configMode >= CM_ROTATE_TO_NAME_ENTRY) && (this->configMode <= CM_NAME_ENTRY_TO_MAIN)) {
		...
		gSPMatrix(
			POLY_OPA_DISP++,
			Matrix_NewMtx(this->state.gfxCtx, "../z_file_choose.c", 2316),
			G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW
		);
	...
	if ((this->configMode >= CM_MAIN_TO_OPTIONS) && (this->configMode <= CM_OPTIONS_TO_MAIN)) {
		...
		gSPMatrix(
			POLY_OPA_DISP++,
			Matrix_NewMtx(this->state.gfxCtx, "../z_file_choose.c", 2337),
			G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW
		);
...
void FileSelect_MoveSelectedFileToTop(GameState* thisx) {
	s16 fileYOffsets[] = { 0, 16, 32 }; // amount to move by to reach the top of the screen
	FileChooseContext* this = (FileChooseContext*)thisx;
	s16 yStep;
...
void FileSelect_SelectModeDraw(GameState* thisx) {
	...
	gSPMatrix(
		POLY_OPA_DISP++,
		Matrix_NewMtx(this->state.gfxCtx, "../z_file_choose.c", 2810),
		G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW
	);
...
void FileSelect_Main(GameState* thisx) {
	...
	if ((this->configMode <= CM_NAME_ENTRY_TO_MAIN) || (this->configMode >= CM_UNUSED_DELAY)) {
		...
		gDPLoadTextureBlock(
			POLY_OPA_DISP++,
			controlsTextures[gSaveContext.language],
			G_IM_FMT_IA,
			G_IM_SIZ_8b,
			144,
			16,
...
void FileSelect_StateInit(GameState* thisx) {
	...
	DmaMgr_SendRequest1(
		...
		(u32)_parameter_staticSegmentRomStart,
		size,
		"../z_file_choose.c",
		3400
	);
