...
void KaleidoScope_DrawDigit(PlayState* play, s32 digit, s32 rectLeft, s32 rectTop) {
    ...
    gDPLoadTextureBlock(
        POLY_OPA_DISP++, ((u8*)gCounterDigit0Tex + (8 * 16 * digit)), G_IM_FMT_I, G_IM_SIZ_8b, 8, 16, 0,
        G_TX_NOMIRROR | G_TX_CLAMP, G_TX_NOMIRROR | G_TX_CLAMP, G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD,
        G_TX_NOLOD
    );
    gSPTextureRectangle(
        POLY_OPA_DISP++, rectLeft << 2, rectTop << 2, (rectLeft + 8) << 2, (rectTop + 16) << 2,
...
void KaleidoScope_DrawDebugEditor(PlayState* play) {
    ...
    gDPSetCombineLERP(
        POLY_OPA_DISP++, 0, 0, 0, PRIMITIVE, TEXEL0, 0, PRIMITIVE, 0, 0, 0, 0, PRIMITIVE, TEXEL0, 0,
        PRIMITIVE, 0
    );
                ...
                if ((i <= SLOT_BOW) || (i == SLOT_SLINGSHOT) || (i == SLOT_BOMBCHU) || (i == SLOT_BEAN)) {
                    ...
                    if (CHECK_BTN_ALL(input->press.button,BTN_CUP)) {
                        ...
                        if (gSaveContext.inventory.items[i] == ITEM_NONE) {
                            ...
                            (gSaveContext.inventory.items[i] >= ITEM_POTION_RED) &&
                            (gSaveContext.inventory.items[i] <= ITEM_POE)
                        ) {
                            gSaveContext.inventory.items[i]--;
                        }
                    ...
                    if (
                        CHECK_BTN_ALL(input->press.button,BTN_CUP) || CHECK_BTN_ALL(input->press.button,BTN_CLEFT) ||
                        CHECK_BTN_ALL(input->press.button,BTN_CDOWN) ||
                        CHECK_BTN_ALL(input->press.button,BTN_CRIGHT)
                    ) {
    ...
    if (curSection == 0) {
        gDPFillRectangle(
            POLY_OPA_DISP++,sSectionPositions[curSection][0],sSectionPositions[curSection][1],
            sSectionPositions[curSection][0] + 45,sSectionPositions[curSection][1] + 16
        );
    } else if ((curSection >= 0x1B) || (curSection == 0x5B)) {
        gDPFillRectangle(
            POLY_OPA_DISP++,sSectionPositions[curSection][0] - 2,sSectionPositions[curSection][1],
            sSectionPositions[curSection][0] + 14,sSectionPositions[curSection][1] + 16
        );
    } else {
        gDPFillRectangle(
            POLY_OPA_DISP++,sSectionPositions[curSection][0] - 4,sSectionPositions[curSection][1],
