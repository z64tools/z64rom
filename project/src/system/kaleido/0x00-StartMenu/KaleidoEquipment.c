...
void KaleidoScope_DrawEquipment(PlayState* play) {
    ...
    if ((pauseCtx->state == 6) && (pauseCtx->unk_1E4 == 0) && (pauseCtx->pageIndex == PAUSE_EQUIP)) {
        ...
        if (
            (pauseCtx->cursorSpecialPos == 0) && (cursorItem != PAUSE_ITEM_NONE) && (pauseCtx->state == 6) &&
            ...
            if (
                ...
                if (pauseCtx->cursorY[PAUSE_EQUIP] == 0) {
                    ...
                    if ((pauseCtx->cursorX[PAUSE_EQUIP] == 3) && (gSaveContext.bgsFlag != 0)) {
                        ...
                        if (
                            (gSaveContext.equips.buttonItems[0] == ITEM_SWORD_BGS) && (gSaveContext.bgsFlag == 0) &&
    ...
    for (rowStart = 0, j = 0, temp = 0, i = 0; i < 4; i++, rowStart += 4, j += 16) {
        ...
        if (LINK_AGE_IN_YEARS == YEARS_CHILD) {
            ...
            if ((i == 0) && (CUR_UPG_VALUE(sAdultUpgrades[i]) == 0)) {
                KaleidoScope_DrawQuadTextureRGBA32(
                    play->state.gfxCtx,
                    gItemIcons[sChildUpgradeItemBases[i] + CUR_UPG_VALUE(sChildUpgrades[i]) - 1],
                    32,
                    32,
                    0
                );
                ...
                KaleidoScope_DrawQuadTextureRGBA32(
                    play->state.gfxCtx,
                    gItemIcons[sAdultUpgradeItemBases[i] + CUR_UPG_VALUE(sAdultUpgrades[i]) - 1],
                    32,
                    32,
                    0
                );
        ...
        for (k = 0, bit = rowStart, point = 4; k < 3; k++, point += 4, temp++, bit++) {
            if (((u32)i == 0) && (k == 2) && (gSaveContext.bgsFlag != 0)) {
                ...
                KaleidoScope_DrawQuadTextureRGBA32(
                    play->state.gfxCtx,
                    gItemIcons[ITEM_SWORD_KOKIRI + temp],
                    32,
                    32,
                    point
                );
