...
void PauseMapMark_DrawForDungeon(PlayState* play) {
    ...
    while (true) {
        ...
        if (
            (mapMarkData->markType == PAUSE_MAP_MARK_BOSS) && (play->sceneId >= SCENE_YDAN_BOSS) &&
        ...
        for (i = 0; i < mapMarkData->count; i++) {
            ...
            if (display) {
                ...
                gDPPipeSync(POLY_OPA_DISP++);
                gDPLoadTextureBlock(
                    POLY_OPA_DISP++, markInfo->texture, markInfo->imageFormat, G_IM_SIZ_MARK,
                ...
                gSPMatrix(
                    POLY_OPA_DISP++, Matrix_NewMtx(play->state.gfxCtx, "../z_lmap_mark.c", 272),
