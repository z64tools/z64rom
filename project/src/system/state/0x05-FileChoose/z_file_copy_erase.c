...
void FileSelect_SelectCopySource(GameState* thisx) {
    ...
    if (
        ((this->buttonIndex == FS_BTN_COPY_QUIT) && CHECK_BTN_ANY(input->press.button, BTN_A | BTN_START)) ||
...
void FileSelect_CopyConfirm(GameState* thisx) {
    ...
    if (
        ((this->buttonIndex != FS_BTN_CONFIRM_YES) && CHECK_BTN_ANY(input->press.button, BTN_A | BTN_START)) ||
        CHECK_BTN_ALL(input->press.button, BTN_B)
    ) {
        this->actionTimer = 8;
        this->nextTitleLabel = FS_TITLE_COPY_TO;
