...
static s32 _enc_z_from_tables(struct yazCtx* ctx, u8* ctl, u8* back, u8* values, u8* output, s32 dec_s, const char* mode) {
	//_enc_next_cpy(NULL);
	u8* b = ctl, * v = values;
...
u32 z64compress_yaz(u8* output, u8* data, u32 data_size) {
	...
	Calloc(ctx, sizeof(struct yazCtx));
	...
	while (pos<sz) {
		s32* search_return = _enc_search(ctx, data, pos, sz, cap);
	...
	output_position = _enc_z_from_tables(ctx, ctx->ctl, ctx->back, ctx->raws, output + 8 + 8, data_size, "Yaz0");
	...
	u32* dataSize = (void*)output;
	
	memcpy(output, "Yaz0", 4);
	dataSize[1] = data_size;
	SwapBE(dataSize[1]);
	
	return output_position + 8 + 8;
}